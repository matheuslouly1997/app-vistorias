...
async function agendaAtivaDaUnidade(supabase: ReturnType<typeof createClient>, unidadeId: string) {
  const { data } = await supabase
    .from("agenda")
    .select("id, tipo, data_agendada, status_agenda")
...
export async function marcarVistoria(input: {
  ...
  const { data: ag, error: errAg } = await supabase
    .from("agenda")
    .insert({
      obra_id: u.obra_id, unidade_id: unidadeId, cliente_id: cliente_id ?? null,
      tipo: "vistoria_1a", data_agendada, duracao_min, observacoes
    } as any)
    .select("id").single();
  ...
  const { error: errU } = await supabase
    .from("unidades")
    .update({ status: "agendado", cliente_atual_id: cliente_id ?? undefined })
    .eq("id", unidadeId);
  if (errU) {
    await supabase.from("agenda").delete().eq("id", ag.id);
...
export async function aprovarUnidade(unidadeId: string): Promise<Resp> {
  ...
  if (agenda) {
    await supabase.from("agenda")
      .update({ status_agenda: "concluida", resultado: "aprovada" })
      .eq("id", agenda.id);
  }
...
export async function reprovarUnidade(unidadeId: string): Promise<Resp> {
  ...
  if (agenda) {
    await supabase.from("agenda")
      .update({ status_agenda: "concluida", resultado: "reprovada" })
      .eq("id", agenda.id);
  }
...
export async function reagendarUnidade(input: {
  ...
  const { data: u } = await supabase
    .from("unidades").select("status, obra_id, cliente_atual_id")
  ...
  const { data: ag, error: errAg } = await supabase
    .from("agenda")
    .insert({
      obra_id: u.obra_id, unidade_id: unidadeId, cliente_id: u.cliente_atual_id,
      tipo: "revistoria", data_agendada, duracao_min, observacoes
    } as any)
    .select("id").single();
  if (errAg) return { erro: errAg.message };
...
export async function resetarUnidade(unidadeId: string): Promise<Resp> {
  const supabase = createClient();
  const { data: u } = await supabase
    .from("unidades").select("status, obra_id").eq("id", unidadeId).maybeSingle();
  if (!u) return { erro: "Unidade nao encontrada" };
  await supabase
    .from("agenda")
  ...
  const { error } = await supabase
    .from("unidades")
    .update({ status: "em_obra", cliente_atual_id: null })
    .eq("id", unidadeId);
  if (error) return { erro: error.message };
  revalidatePath("/obras", "layout");
  return { ok: true };
...
export async function desfazerUltimaAlteracao(unidadeId: string): Promise<Resp & { para?: StatusUnidade }> {
  ...
  const { data: u } = await supabase
    .from("unidades").select("status").eq("id", unidadeId).maybeSingle();
  if (!u) return { erro: "Unidade nao encontrada" };
  ...
  if (alvo === "agendado" || alvo === "revistoria") {
    const { data: ag } = await supabase
      ...
      .select("id")
      .eq("unidade_id", unidadeId)
      .eq("status_agenda", "concluida")
      .order("data_agendada", { ascending: false })
      .limit(1);
    if (ag?.[0]) {
      await supabase.from("agenda")
        .update({ status_agenda: "agendada", resultado: null })
        .eq("id", ag[0].id);
    }
  }
