...
export default function ControleTermos({
  ...
  async function handleUpload(unidadeId: string, agendaId: string | null) {
    ...
    try {
      ...
      const r = await salvarTermo({ unidadeId, agendaId, resultado: resultadoUpload, arquivoPath: path });
      if (r.erro) {
        await supabase.storage.from("termos-unidade").remove([path]);
        toast.erro(r.erro); return;
      }
      setAnexandoId(null);
      setArquivo(null);
    ...
    <div className="max-w-full mx-auto px-4 py-6 space-y-4">
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            ...
            {qtdDivergente > 0 && (
              <span className="ml-1 text-amber-700 font-medium">
                · {qtdDivergente} divergente{qtdDivergente > 1 ? "s" : ""}
              </span>
            )}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {torresNomes.length > 1 && (
          <select value={filtroTorre} onChange={(e) => setFiltroTorre(e.target.value)} className="border rounded-md px-2 py-1.5 text-sm bg-white">
            <option value="todas">Todas as torres</option>
            {torresNomes.map((n) => <option key={n} value={n}>{n}</option>)}
          </select>
        )}

        <select value={filtroStatus} onChange={(e) => setFiltroStatus(e.target.value)} className="border rounded-md px-2 py-1.5 text-sm bg-white">
          <option value="todas">Todos os status</option>
          {statusUnicos.map((s) => <option key={s} value={s}>{STATUS_LABELS[s] ?? s}</option>)}
        </select>

        {(filtroTermo !== "todas" || filtroTorre !== "todas" || filtroStatus !== "todas") && (
          <button onClick={() => { setFiltroTermo("todas"); setFiltroTorre("todas"); setFiltroStatus("todas"); }} className="text-sm text-gray-500 hover:text-gray-900 underline">
            Limpar filtros
          </button>
        )}
      </div>
        ...
        <table className="min-w-full text-sm divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-3 py-2.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">Ações</th>
            </tr>
          </thead>
                  ...
                  <td className="px-3 py-2.5">
                    {anexandoId === f.unidadeId ? (
                      <div className="flex flex-col gap-1.5 min-w-[220px]">
                        <select value={resultadoUpload} onChange={(e) => setResultadoUpload(e.target.value as any)} className="border rounded px-2 py-1 text-xs bg-white">
                          <option value="aprovacao">Aprovação</option>
                          <option value="reprovacao">Reprovação</option>
                        </select>
                        <input type="file" accept=".pdf,image/*" onChange={(e) => setArquivo(e.target.files?.[0] ?? null)} className="text-xs" />
                        <div className="flex gap-1">
                          <button onClick={() => handleUpload(f.unidadeId, f.agendaId)} disabled={!arquivo || uploading} className="text-xs px-2 py-1 rounded bg-blue-600 text-white disabled:opacity-50">
                            {uploading ? "Enviando..." : "Salvar"}
                          </button>
                          <button onClick={() => { setAnexandoId(null); setArquivo(null); }} className="text-xs px-2 py-1 rounded border">
                            Cancelar
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {f.temTermo && f.termoArquivoPath && (
                          <>
                            <button onClick={() => handleBaixar(f.termoArquivoPath!)} className="text-xs px-2 py-1 rounded border text-blue-700 hover:bg-blue-50 whitespace-nowrap">
                              Visualizar
                            </button>
                            <button
                              onClick={() => confirm(`Remover o termo da unidade ${f.identificador}?`) && handleExcluir(f.termoId!, f.termoArquivoPath!)}
